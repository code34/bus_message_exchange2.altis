	...
	*/
	...
	BME_fnc_publicvariable = {
		...
		if!(typename _remotefunction == "STRING") exitwith {"BME: wrong type variablename parameter, should be STRING" call BME_fnc_log;};
		if(isnil "_variable") exitwith {format["BME:  Variable data for %1 handler is nil", _remotefunction] call BME_fnc_log;};
		...
		if(isNil "_playerid") then {
			bme_send_queue pushBack [_remotefunction, _variable, _destination];
		} else {
			bme_send_queue pushBack [_remotefunction, _variable, _destination, _playerid];
		};
	};
	...
	BME_fnc_addqueue = {
		...
		if((isserver) and ((_destination == "server") or (_destination == "all"))) then {
			bme_queue pushBack [_this select 0, _this select 1, "server"];
		};
		...
		if((local player) and ((_destination == "client") or (_destination == "all"))) then {	
			bme_queue pushBack [_this select 0, _this select 1, "client"];
		};
	};
